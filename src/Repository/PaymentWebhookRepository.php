...
use BlackCat\Database\Packages\PaymentWebhooks\Repository\PaymentWebhookRepositoryInterface;
...
class PaymentWebhookRepository implements PaymentWebhookRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\PaymentWebhooks\Definitions::class);
    ...
    public function getByPayloadHash(string $payloadHash, bool $asDto = false): array|\BlackCat\Database\Packages\PaymentWebhooks\Dto\PaymentWebhookDto|null {
        return $this->getByUnique([ 'payload_hash' => $payloadHash ], $asDto);
    }
    ...
    public function getIdByPayloadHash(string $payloadHash) {
        $row = $this->getByPayloadHash($payloadHash, false);
