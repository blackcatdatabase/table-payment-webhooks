...
namespace BlackCat\Database\Packages\PaymentWebhooks\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\PaymentWebhooks\Definitions;
use BlackCat\Database\Packages\PaymentWebhooks\Criteria;
use BlackCat\Database\Packages\PaymentWebhooks\Dto\PaymentWebhookDto as Dto;
use BlackCat\Database\Packages\PaymentWebhooks\Mapper\PaymentWebhookDtoMapper as RowMapper;
...
final class PaymentWebhookRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\PaymentWebhooks\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\PaymentWebhooks\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\PaymentWebhooks\Dto\PaymentWebhookDto|null */
    public function getByPayloadHash(string $payloadHash, bool $asDto = false): array|\BlackCat\Database\Packages\PaymentWebhooks\Dto\PaymentWebhookDto|null {
        $row = $this->getByUnique([ 'payload_hash' => $payloadHash ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\PaymentWebhooks\Mapper\PaymentWebhookDtoMapper::fromRow($row);
    }
    public function existsByPayloadHash(string $payloadHash): bool {
        $where = 't.' . Ident::q($this->db, 'payload_hash') . ' = :uniq_payload_hash';
        return $this->exists($where, [ 'uniq_payload_hash' => $payloadHash ]);
    }
    /** @return int|string|null */
    public function getIdByPayloadHash(string $payloadHash) {
        $row = $this->getByPayloadHash($payloadHash);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\PaymentWebhooks\Dto\PaymentWebhookDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\PaymentWebhooks\Dto\PaymentWebhookDto|null {
