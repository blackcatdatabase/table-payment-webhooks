...
namespace BlackCat\Database\Packages\PaymentWebhooks\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\PaymentWebhooks\Dto\PaymentWebhookDto;
use BlackCat\Database\Packages\PaymentWebhooks\Mapper\PaymentWebhookDtoMapper;
 ...
 */
final class PaymentWebhooksAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private PaymentWebhookRepository $paymentWebhookRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'payment_webhooks' => $this->paymentWebhookRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\PaymentWebhooks\ModuleException("Record {$idStr} not found for locking.");
            }
