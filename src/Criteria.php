...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [ 'id', 'payment_id', 'gateway_event_id', 'payload_hash', 'payload', 'from_cache', 'created_at' ];
    }

    /** Columns used for full-text LIKE/ILIKE searches. */
    protected function searchable(): array
    {
        return [ 'gateway_event_id', 'payload_hash' ];
    }

/** Columns allowed in ORDER BY (falls back to filterable() when empty). */
protected function sortable(): array
{
    return [ 'id', 'payment_id', 'gateway_event_id', 'payload_hash', 'from_cache', 'created_at' ];
}
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\PaymentWebhooks\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\PaymentWebhooks\Definitions::softDeleteColumn();
