 ...
 * - whitelist filtrů: [ 'id', 'payment_id', 'gateway_event_id', 'payload_hash', 'payload', 'from_cache', 'created_at' ]
 * - whitelist pro LIKE hledání: [ 'gateway_event_id', 'payload_hash' ]
 */
final class Criteria {
    ...
    public function toSql(bool $viewMode = false): array {
        ...
        // filtry (rovnosti / IN)
        $allowed = array_fill_keys([ 'id', 'payment_id', 'gateway_event_id', 'payload_hash', 'payload', 'from_cache', 'created_at' ], true);
        ...
        if ($this->search !== null) {
            $searchCols = [ 'gateway_event_id', 'payload_hash' ];
            $likeParts = [];
        ...
        $order = null;
        if ($this->sort) {
            $safeCols = array_fill_keys([ 'id', 'payment_id', 'gateway_event_id', 'payload_hash', 'payload', 'from_cache', 'created_at' ], true);
            $parts = [];
